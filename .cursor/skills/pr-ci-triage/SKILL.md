...
See [docs/TOOLS.md](../../docs/TOOLS.md) for all **52** business tools (+ 3 lazy meta-tools).
...
| `pr_get_ci_snapshot` | CI_KIND + failing runs + digest per run (lightest multi-run) | `repo`, `pr_number` |
| `ci_analyze_pr_failures` | Failing Actions runs + external/pending checks; first line `CI_KIND` | `repo`, `pr_number` |
...
2. **`pr_get_ci_snapshot`** (preferred) or **`ci_analyze_pr_failures`** — read **`CI_KIND`** first:
   - `actions_only` or `mixed` → continue with run IDs below
   - `external_only` / `pending` → PR checks tab; **do not** call `ci_get_failed_logs`
