...
server's own code — use the unistar-mcp MCP tools, not raw `gh`/`git`. The
`pr-ci-triage` skill (`.cursor/skills/pr-ci-triage/SKILL.md`) documents the intended
tool-chaining workflow.

### Cursor setup

- **`.cursor/mcp.json`** registers the server with `go run ./cmd` — works from a fresh
  clone (Go required). Open **Settings → Tools & MCP** and enable `unistar-mcp` on
  first use.
...
See README § "Use with Claude Code" for details. `gh pr`/`gh run` are denied in
...
- `pkg/server/` — the MCP server itself:
  ...
  - `pr_chat_tools.go` — chat-oriented PR tools (`pr_get_overview`, `pr_get_merge_blockers`, …).
...
Doc drift is guarded by `pkg/server/doc_test.go` (`TestDocsMentionAllRegisteredTools`).
